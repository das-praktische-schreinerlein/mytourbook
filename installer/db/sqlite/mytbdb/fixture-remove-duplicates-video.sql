...
where
    video_keyword.kw_id = doubletteKeywords.old_kw_id
    AND video_keyword.kw_id in
      (
...
CREATE TEMPORARY TABLE IF NOT EXISTS tmp_video_keyword AS
    SELECT * FROM video_keyword where true = false;
...
DELETE FROM video_keyword
WHERE vk_id in
      (
      SELECT video_keyword.vk_id
