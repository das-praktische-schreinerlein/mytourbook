...
UPDATE tour
SET
  t_datefirst=grouped.min_datevon,
  t_datelast=grouped.max_datevon
FROM
  (SELECT t_id, MIN(min_datevon) AS min_datevon, MAX(max_datevon) AS max_datevon
   FROM
    (
...
UPDATE poi
SET
    poi_datefirst=grouped.min_datevon,
    poi_datelast=grouped.max_datevon
FROM
     (SELECT poi_id, MIN(min_datevon) AS min_datevon,  MAX(max_datevon) AS max_datevon
      FROM
          (
...
WHERE poi.poi_id=grouped.poi_id;
          ...
          (
                ...
                UNION
                  SELECT
                   ...
                   GROUP BY l_lochirarchietxt, lh.l_id
          ) getLocs on INSTR(getLocs.l_lochirarchietxt, lh.l_lochirarchietxt) > 0
      GROUP BY lh.l_lochirarchietxt ORDER BY lh.l_lochirarchietxt DESC
     ) grouped
...
UPDATE location
SET
    l_datefirst=grouped.min_datevon,
    l_datelast=grouped.max_datevon
FROM location_hirarchical lh,
     (SELECT lh.l_lochirarchietxt, MIN(min_datevon) AS min_datevon, MAX(max_datevon) AS max_datevon
      FROM location_hirarchical lh INNER JOIN
          (
                  SELECT
...
WHERE location.l_id=lh.l_id AND grouped.l_lochirarchietxt = lh.l_lochirarchietxt;
