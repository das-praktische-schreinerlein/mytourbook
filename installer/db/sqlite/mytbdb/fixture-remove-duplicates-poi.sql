...
where
    poi_keyword.kw_id = doubletteKeywords.old_kw_id
    AND poi_keyword.kw_id in
      (
...
CREATE TEMPORARY TABLE IF NOT EXISTS tmp_poi_keyword AS
    SELECT * FROM poi_keyword where true = false;
...
DELETE FROM poi_keyword
WHERE poik_id in
      (
      SELECT poi_keyword.poik_id
