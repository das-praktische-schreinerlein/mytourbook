...
where
    kategorie_keyword.kw_id = doubletteKeywords.old_kw_id
    AND kategorie_keyword.kw_id in
      (
...
CREATE TEMPORARY TABLE IF NOT EXISTS tmp_kategorie_keyword AS
    SELECT * FROM kategorie_keyword where true = false;
...
DELETE FROM kategorie_keyword
WHERE kk_id in
      (
      SELECT kategorie_keyword.kk_id
