...
select joined_keyword.kw_id as old_kw_id, keyword.kw_name, doubleKeywords.new_kw_id, doubleKeywords.all_kw_id from tour_keyword as joined_keyword
...
SELECT distinct c1.* FROM tour_keyword c1
INNER JOIN tour_keyword c2
WHERE
    c1.tk_id > c2.tk_id AND
    c1.t_id = c2.t_id AND
    c1.kw_id = c2.kw_id
LIMIT 10;


CREATE TEMPORARY TABLE IF NOT EXISTS tmp_tour_keyword AS (SELECT *
                                                          FROM tour_keyword
                                                          where true = false);
