...
@Injectable()
export class TourDocRouteDescSuggesterService extends CommonDocListSuggesterService<TourDocRecord, TourDocSearchForm, TourDocSearchResult> {
    ...
    protected appendFiltersToListItemSearchForm(searchForm: TourDocSearchForm, form: {}, environment: CommonDocListSuggesterEnvironment) {
        ...
        searchForm.moreFilter = 'route_id_is:' + (form['id'] ? form['id'].replace(/[-a-zA-Z_]+/g, '') : 999999999999999);
    }

    protected getConfiguration(environment: CommonDocListSuggesterEnvironment): CommonDocListSuggesterConfiguration {
        return BeanUtils.getValue(this.appService.getAppConfig(), 'components.tdoc-route-desc-suggester') || {
            ...
            headingTemplate: '**Was für eine Tour**\n\n'
        };
    }
