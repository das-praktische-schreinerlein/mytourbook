export class TourDocSqlUtils {
    ...
    public static transformToSqliteDialect(sql: string): string {
        ...
        sql = sql.replace(/\(SELECT CONCAT\("type=mainroute:::name=", COALESCE\(t_name, "null"\), ":::refId=", CAST\(tour.t_id AS CHAR\),   ":::full=true:::linkedRouteAttr=", COALESCE\(kategorie_tour.kt_route_attr, "null"\)\)/g,
