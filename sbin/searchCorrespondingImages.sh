...
if [ "${USESIMILARITYINDEX}" != "" ]; then
    ...
    node dist/backend/serverAdmin.js\
           ...
           --action findCorrespondingTourDocRecordsForMedia\
           --importDir $SEARCHDIR\
           --additionalMappingsFile $SEARCHDIR/findFilesInLireIndex.json\
           --debug true\
           --outputFile $SEARCHDIR/findFilesInDb.json > "$SEARCHDIR/findFilesInDb.log"
    cd ${CWD}
else
