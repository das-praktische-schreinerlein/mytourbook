...
# check parameters
if [ "$#" -lt 1 ]; then
    dofail "USAGE: readImageMetaData.sh CONFIGPROFILE] \nFATAL: requires 1 parameters " 1
    exit 1
fi
...
echo "start - readImageMetaData configProfile='${CONFIGPROFILE}'"
cd ${MYTB}

node dist/backend/serverAdmin.js\
    --debug 10\
    --command mediaManager\
    --action readImageMetaData\
    --adminclibackend "${CLICONFGFILE}"\
    --backend "${CONFGFILE}"\
    --force "${FORCE}"\
    --parallel 1

echo "done - readImageMetaData configProfile='${CONFIGPROFILE}'"
